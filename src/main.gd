extends Node

signal game_over

const SHIP_SCENE := preload("res://src/ship/ship.tscn")

@export var ship_spawn_radius: float
@export var low_time_threshold: float

@export var world: Node2D
@export var camera: ShakeCamera2D
@export var ship_spawn_cast: ShapeCast2D
@export var wind: Wind
@export var music: Music
@export var hud_layer: CanvasLayer
@export var time_limit_label: Label
@export var score_label: RichTextLabel
@export var game_timer: Timer

var is_game_over := false

var game_active: bool:
	set(value):
		game_active = value
		hud_layer.visible = game_active
		get_tree().paused = not game_active


func move_ship_spawn_cast() -> void:
	ship_spawn_cast.position = Vector2(
		ship_spawn_radius * sqrt(randf()),
		0,
	).rotated(randf() * TAU)
	ship_spawn_cast.force_shapecast_update()


func find_ship_spawn_position() -> Vector2:
	move_ship_spawn_cast()
	while ship_spawn_cast.get_collision_count() > 0:
		move_ship_spawn_cast()
	return ship_spawn_cast.position


func spawn_ship(details: PlayerDetails) -> void:
	var spawn_position := find_ship_spawn_position()
	var ship: Ship = SHIP_SCENE.instantiate()
	ship.details = details
	ship.position = spawn_position
	ship.rotation = randf() * TAU
	ship.cannon_fired.connect(music._on_ship_cannon_fired)
	ship.damage_taken.connect(music._on_ship_damage_taken)
	ship.cannon_fired.connect(camera._on_ship_cannon_fired)
	ship.damage_taken.connect(camera._on_ship_damage_taken)
	ship.destroyed.connect(_on_ship_destroyed)
	world.add_child.call_deferred(ship)
	ship.wind = wind


func _ready() -> void:
	game_active = false
	SignalBus.node_spawned.connect(_on_node_spawned)


func _process(delta: float) -> void:
	var total_seconds_left := ceili(game_timer.time_left)
	var minutes_left := total_seconds_left / 60
	var seconds_left := total_seconds_left % 60
	time_limit_label.text = "%d:%02d" % [minutes_left, seconds_left]
	if total_seconds_left < Globals.time_limit_seconds * low_time_threshold:
		time_limit_label.modulate = Color(1, 0.25, 0.25)
	else:
		time_limit_label.modulate = Color.WHITE


func sort_score_descending(
		details1: PlayerDetails,
		details2: PlayerDetails,
) -> bool:
	return details1.score > details2.score


func update_score_label() -> void:
	if is_game_over:
		return

	score_label.clear()
	var details_list := Globals.players.values()
	details_list.sort_custom(sort_score_descending)
	for i in range(len(details_list)):
		var details: PlayerDetails = details_list[i]
		var highest_score: bool = (
			i == 0
			and len(details_list) > 1
			and details_list[1].score < details.score
		)
		if highest_score:
			score_label.push_color(Color(1, 0.75, 0))
		score_label.add_text(
			"%d: %s\n" % [details.score, details.nickname],
		)
		if highest_score:
			score_label.pop()


func start() -> void:
	for details_resource in Globals.players.values():
		var details := details_resource as PlayerDetails
		spawn_ship(details)
	is_game_over = false
	game_active = true
	if Globals.time_limit_seconds > 0:
		game_timer.paused = false
		game_timer.start(Globals.time_limit_seconds)
		time_limit_label.show()
	else:
		time_limit_label.hide()
	score_label.visible = Globals.game_mode == Globals.GameMode.DEATHMATCH
	update_score_label()


func end() -> void:
	for details in Globals.players.values():
		details.score = 0
	# We could also try to free splashes and impacts here, but they have short
	# lifespans and don't affect the game
	get_tree().call_group(&"ships", &"queue_free")
	get_tree().call_group(&"cannonballs", &"queue_free")
	game_active = false


func signal_game_over() -> void:
	game_over.emit()
	is_game_over = true
	game_timer.paused = true


func _on_ship_destroyed(ship: Ship, destroyer: int) -> void:
	if is_game_over:
		return

	if Globals.game_mode == Globals.GameMode.LAST_MAN_STANDING:
		var ships := get_tree().get_nodes_in_group(&"ships")
		# Subtract 1 to account for the ship that was just destroyed
		if len(ships) - 1 <= 1:
			signal_game_over()
	elif Globals.game_mode == Globals.GameMode.DEATHMATCH:
		var highest_new_score: int
		if destroyer == Globals.NO_PLAYER or destroyer == ship.details.player:
			ship.details.score += Globals.deathmatch_self_destruct_score
			highest_new_score = ship.details.score
		else:
			var destroyer_details := Globals.players[destroyer] as PlayerDetails
			destroyer_details.score += Globals.deathmatch_kill_score
			ship.details.score += Globals.deathmatch_death_score
			highest_new_score = max(ship.details.score, destroyer_details.score)
		update_score_label()
		if Globals.score_limit > 0 and highest_new_score >= Globals.score_limit:
			signal_game_over()
		if not is_game_over:
			spawn_ship(ship.details)


func _on_game_timer_timeout() -> void:
	signal_game_over()


func _on_lobby_menu_play_pressed(previous: Menu) -> void:
	start()


func _on_game_over_menu_menu_pressed(previous: Menu) -> void:
	end()


func _on_pause_menu_menu_pressed(previous: Menu) -> void:
	end()


func _on_node_spawned(node: Node2D) -> void:
	world.add_child(node)
